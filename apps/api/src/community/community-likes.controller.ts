...
/**
 * Tight per-route limits on the community write surfaces (the global default is
 * a loose 120/min meant for reads). Likes allow honest toggling; reports are a
 * deliberate act — nobody legitimately files more than a few per minute.
 */
const LIKE_THROTTLE = { default: { limit: 20, ttl: 60_000 } } as const;
const REPORT_THROTTLE = { default: { limit: 5, ttl: 60_000 } } as const;
